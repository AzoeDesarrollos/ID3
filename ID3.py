...
def lengthen(strng, num_spaces):
    strng = strng[:num_spaces]
    return strng + (' ' * (num_spaces - len(strng)))
...
class ID3:
    ...
    def __setitem__(self, k, v):
        key = k
        if key in ['TITLE', 'ARTIST', 'ALBUM', 'YEAR', 'COMMENT', 'TRACKNUMBER', 'GENRE']:
            if k == 'TRACKNUMBER':
                if type(v) is int:
                    self.track = v
                else:
    ...
    def __str__(self):
        if self.has_tag:
            if self.genre is not None and 0 <= self.genre < len(self.genres):
            ...
            return "File   : %s\nTitle  : %-30.30s  Artist: %-30.30s\nAlbum  : %-30.30s  Track : %s  Year: " \
                   "%-4.4s\nComment: %-30.30s  Genre : %s (%i)" % (self.filename, self.title, self.artist,
                                                                   self.album, track, self.year, self.comment, genre,
                                                                   self.genre)
        else:
    ...
    def __setattr__(self, name, value):
        if name in ['title', 'artist', 'album', 'year', 'comment', 'track', 'genre']:
            self.__dict__['modified'] = 1
            self.__dict__['has_tag'] = 1
            if name == 'track':
